...
STYLE = {
    "35": {"label": "35kV", "color": "#2A9D74", "width": 0.42, "alpha": 0.76, "z": 2},
    "110": {"label": "110kV", "color": "#2F6FD6", "width": 0.54, "alpha": 0.80, "z": 3},
    "220": {"label": "220kV", "color": "#D69A18", "width": 0.78, "alpha": 0.86, "z": 4},
    "other_dc": {"label": "其他直流", "color": "#7B61A8", "width": 0.92, "alpha": 0.90, "z": 5},
    "500plus": {"label": "500kV及以上", "color": "#E0525E", "width": 1.48, "alpha": 0.95, "z": 6},
}
...
plt.rcParams.update(
    {
        "font.family": ["SimHei", "PingFang SC", "Arial Unicode MS"],
...
def draw_map():
    ...
    ax.add_collection(LineCollection(district_xy, colors=DISTRICT, linewidths=0.36, alpha=0.72, zorder=1))
    ...
    ax.add_collection(LineCollection(province_xy, colors=OUTER, linewidths=0.86, alpha=0.95, zorder=8))
    ...
    fig.text(0.045, 0.965, "江苏省域输电线路电压等级分布", fontsize=15.5, weight="bold", color=TEXT, va="top")
    fig.text(
        0.045,
        0.925,
    ...
    handles = [
        Line2D([0], [0], color=STYLE[key]["color"], lw=max(STYLE[key]["width"] * 2.0, 1.3), label=STYLE[key]["label"])
    ...
    bar_x1 = max_x - 0.31 * (max_x - min_x)
    bar_x2 = bar_x1 + scale_projected
    ax.plot([bar_x1, bar_x2], [bar_y, bar_y], color=TEXT, lw=1.5, solid_capstyle="butt", zorder=10)
    ax.plot([bar_x1, bar_x1], [bar_y - 5000, bar_y + 5000], color=TEXT, lw=0.8, zorder=10)
    ax.plot([bar_x2, bar_x2], [bar_y - 5000, bar_y + 5000], color=TEXT, lw=0.8, zorder=10)
    ax.text((bar_x1 + bar_x2) / 2, bar_y + 8500, "100 km", color=TEXT, fontsize=7.8, ha="center", va="bottom")
