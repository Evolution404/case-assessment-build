...
def build():
    ...
    para(doc,"4,282对“确认不同”同样具有价值。它们覆盖了同位置、同设备、构图接近但现场确有变化的正常场景，可与确认重复样本一起用于检查阈值是否偏向某一类画面。水印修改、裁剪、压缩和高相似不同等典型正反样本持续保留，使参数调整既关注重复照片能否被召回，也关注人工复核量是否保持在合理范围。")
    ...
    para(doc,"交叉跨越、防鸟和集中燃放点共用线路空间底座，告警工单和巡视照片共用照片智能筛查方法，两个方向最终都落到同一管理动作：机器处理规模大、重复性高的筛选工作，外协人员和主业人员集中完成现场确认、业务终审和问题处置。相关成果已经进入省公司实际管理流程，并多次形成生产管控中心管理通报。")
    ...
    para(doc,"从应用路径看，这项工作经历了三次放大。第一次是把交叉跨越这一单项排查做成批量空间分析，解决一个具体任务；第二次是复用同一线路空间底座，将方法延伸到防鸟和集中燃放点，证明数据和规则可以跨专项复用；第三次是把照片筛查接入省公司实际管理，使数字化能力从“个人解决问题”进入“组织持续使用”的阶段。三个阶段对应了从单项增效、方法复用到管理机制沉淀的完整过程。")
    ...
    h2(doc,"结语")
    para(doc,"回看整个案例，有两组变化最能说明这条路径的价值。任务侧，一次原需十余人、近两周完成的交叉跨越排查，第一轮全量计算现在只需数分钟，同时补充发现十余处人工漏项；质量侧，照片查重从告警工单起步，随后扩展到巡视照片，并在省公司试点中发现500千伏特殊通道重复用图这一较大运检质量问题。两个场景一前一后，分别解决了“任务太多找不过来”和“照片太多查不过来”的现实矛盾。",14,False,together=True)
