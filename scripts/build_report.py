...
from build_report_figure10 import build as build_report_figure10
from build_report_figure11 import build as build_report_figure11
...
def figure(doc,name,caption,width=15.0):
    confirmed={
        ...
        "photo-edit-case":"09-告警工单反馈照片与告警照片对照.png",
        "management-evidence":"10-照片查重管理通报证据.png",
    }
...
def build():
    ...
    figure(doc,"photo-edit-case","图10　告警工单反馈照片与对应告警照片对照",15.2)
    para(doc,"如图10所示，5月22日告警照片中现场人员持伞，5月25日告警照片中现场人员未持伞；但两次工单的反馈照片整体构图高度相似。结合告警照片中的人物区域局部放大对照，5月25日反馈照片疑似复用了5月22日的历史反馈画面，并对雨伞区域进行了局部图像编辑后再次提交。该案例表明，仅检查是否上传反馈照片难以及时发现经过局部修改的历史照片复用问题，需要结合跨时间照片查重与原始告警照片核查，才能形成有效的履职质量监督。")
    para(doc,"2026年第14期《生产管控中心运检工作质量远程督查周报-输电分册》通报了一项500千伏特殊区段人工巡视照片重复问题。管理人员调取两次巡视原始记录逐项核对，确认同一区段两次人工巡视使用了重复照片；周报明确认定为人工巡视不到位，问题等级为“较大（运检质量类）”。")
    para(doc,"截至2026年第21期，照片查重发现的问题已在10期正式周报中形成管理通报，累计19项，其中告警工单反馈照片重复11项、人工巡视照片重复8项。")
    page_break(doc)
    # 14 algorithm
    ...
    para(doc,f"告警工单照片筛查从{CFG['metrics']['alarm_photos']:,}张反馈照片起步，形成{CFG['metrics']['alarm_candidates']:,}对疑似相似照片，目前已确认重复{CFG['metrics']['alarm_confirmed_pairs']:,}对。随后，照片查重能力扩展到巡视业务和全电压等级。截至2026年第21期，相关结果已在10期正式周报中形成照片重复类问题{CFG['metrics']['province_trial_duplicate_issues']}项，其中告警工单反馈照片重复{CFG['metrics']['province_trial_alarm_duplicate_issues']}项、人工巡视照片重复{CFG['metrics']['province_trial_patrol_duplicate_issues']}项。")
    para(doc,"其中，2026年第14期周报通报一项500千伏特殊区段人工巡视照片重复问题，明确认定为人工巡视不到位，问题等级为“较大（运检质量类）”。这说明照片查重结果已经从异常候选进入正式质量评价和管理闭环。")
    figure(doc,"management-evidence","图11　照片查重结果进入省公司生产管控中心管理通报",15.2)
    h2(doc,"（三）管理方式：机器处理全量，人员负责判断")
    para(doc,"交叉跨越、防鸟和集中燃放点共用线路空间底座，告警工单和巡视照片共用照片智能筛查方法。机器处理规模大、重复性高的筛选工作，外协人员和主业人员集中完成现场确认、业务终审和问题处置；照片查重已用于省公司生产管控中心运检工作质量远程督查并形成正式周报通报。")
    ...
    paper_table(doc,"表10　案例实施成效与管理方式变化",("管理环节","传统方式","数智协同方式","实际成效"),(
        ...
        ("外协履职监督","依赖有限抽查和人员历史记忆","照片全量筛查后集中复核异常候选","截至2026年第21期，10期正式周报累计通报19项照片重复问题，其中1项500千伏特殊区段问题定性为较大运检质量问题"),
    ...
    para(doc,"应用范围由交叉跨越逐步扩展到防鸟、集中燃放点和照片查重，其中照片查重已进入省公司生产管控中心运检工作质量远程督查并形成正式周报通报。空间专项复用线路空间底座，照片查重复用统一筛查流程。")
    ...
    para(doc,"下一步结合省公司实际应用情况，把现场核验结果、设备变更信息和照片复核结论及时回写，并同步记录专项排查耗时、候选核验量和异常发现数量。推广前重点统一数据接口、规则参数和处理流程，再向其他地市公司推广应用。")
    ...
    h2(doc,"结语")
    para(doc,"本案例最直接的成效体现在两个方面。任务侧，一次原需十余人、近两周完成的交叉跨越排查，第一轮全量计算现在只需数分钟，同时补充发现十余处人工漏项；质量侧，照片查重从告警工单起步，随后扩展到巡视照片，截至2026年第21期已在10期正式周报中形成19项照片重复问题，其中1项500千伏特殊区段问题定性为较大运检质量问题。两个场景分别解决了“任务太多找不过来”和“照片太多查不过来”的现实矛盾。",14,False,together=True)
