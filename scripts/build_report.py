...
def build():
    ...
    para(doc,"针对上述问题，我从输电运检实际业务出发，自主开展数据梳理、规则设计和数字化工具开发，把空间计算用于任务筛选，把图像相似度识别用于照片查重，并归纳为“增效管任务、提质管履职”两条主线：任务侧由系统从全量对象中筛选重点，外协按清单精准核验；履职侧由系统对作业照片全量查重，管理人员重点复核疑似异常照片对。相关算法、计算工具和业务筛选流程均为自主设计、开发和验证成果。")
    ...
    para(doc,"为了避免不同专项各自维护一套数据，本案例把业务对象按“实体—关系—状态”组织。线路、杆塔、外部点位、工单和照片属于实体；线路与杆塔、工单与照片、候选照片A与B之间属于关系；是否待核验、是否确认重复、是否已闭环属于状态。数据处理时尽量使用稳定标识关联对象，展示给人员时再恢复线路名称、杆号和业务描述。这样可以降低名称变更、表格列顺序变化对计算链的影响。")
    ...
    para(doc,"二是把业务经验转化为可计算规则。在统一数据底座基础上，将“是否相交”“是否进入重点活动区域”“是否落入安全距离”等业务判断转化为空间叠加、风险分级和距离阈值，并编写对应计算逻辑，把个人经验固化为可重复执行的筛选规则，批量生成重点核验清单。")
    ...
    para(doc,"式（1）用于说明有限线段求交的几何原理，其中A、B为输电线路段两个端点，C、D为外部线状要素对应线段的两个端点，t、u为各自线段参数；当两条参数线段存在共同点且参数落在0到1区间内时，交点位于两条有限线段上。实际精确求交由Shapely几何运算完成，公式用于解释判定原理。",together=True)
    ...
    para(doc,"在某地市实际应用中，原需十余人、近两周完成的全量排查，空间分析工具数分钟即可完成，同时还补充识别出十余处此前人工排查遗漏的铁路、公路等重要跨越信息。这次排查也让我确认，线路空间数据一旦整理好，后续专项可以直接复用，不必每次重新从台账和地图开始查找。")
    ...
    para(doc,"活动等级采用样本分布的分位数划分。对平滑后大于0的有效栅格，分析中分别取70%、84%、94%分位点作为活动区域进入阈值、较活跃分界和高活跃分界，再将活动表面转换到与线路一致的平面坐标进行叠加。分位数规则能够随当前输入记录的空间分布变化，避免长期固定一个记录数量阈值。最终线路筛选仍保留现场核验，GBIF发生记录只负责把大范围线路收敛到更值得关注的区段。")
    ...
    para(doc,"三个空间专项可以用“点—线—面”三个空间对象统一理解。交叉跨越对应“线”，重点判断输电线路与铁路、公路等线状要素是否相交；防鸟对应“面”，先将鸟类发生记录聚合为活动重点区域，再与输电线路叠加筛选重点区段；集中燃放点对应“点”，以燃放位置为中心，按业务距离筛选周边杆塔并汇总涉及线路。三个场景分别从线、面、点切入，共用同一套线路杆塔空间底座和现场核验流程。")
    ...
    ),widths=(2.5,2.5,5.2,4.1),note="三类专项分别以线、面、点为分析入口，共用线路与杆塔空间底座，业务差异主要体现在筛选规则和核验重点。")
    page_break(doc)
    # 12 quality problem
    ...
    para(doc,"真正推动照片查重进入实际管理的，是省公司试点中的一次500千伏特殊通道复核。照片智能筛查模块先筛出一组高相似照片，管理人员随即调取原始巡视记录逐项核对，最终确认同一区段两次人工巡视使用了重复照片。由于该记录无法真实反映当时通道情况，相关问题被认定为人工巡视不到位，并定性为较大运检质量问题。这一过程完整走过了“机器筛出线索—人工调取原始记录—业务确认—形成管理结论”的链条，这一案例成为照片查重进入实际管理的典型节点。")
    ...
    para(doc,"111,519张反馈照片经过分层筛选后形成5,472对疑似相似照片，已有4,630对完成人工复核，其中确认重复348对、确认不同4,282对，另有842对待复核。若不附加任何业务约束，这批照片的理论两两组合约62.18亿对；这一数量级只用于说明原始搜索空间，实际处理通过候选生成把人工和高成本计算集中到少量照片对上。348对确认重复占已复核候选的7.52%，这一比例反映候选队列的复核结果，不代表全部照片的重复率。")
    para(doc,"4,282对“确认不同”同样具有价值。它们覆盖了同位置、同设备、构图接近但现场确有变化的正常场景，可与确认重复样本一起用于检查阈值是否偏向某一类画面。水印修改、裁剪压缩和高相似不同等典型正反样本持续保留，使参数调整既关注重复照片能否被召回，也关注人工复核量是否保持在合理范围。")
    ...
    para(doc,"表5已经把照片标准化、pHash召回、CLIP复核、候选归并和人工终审串成完整链路。业务应用时，管理人员直接查看带有相似度、任务对象和时间信息的疑似照片对，复核人员重点核对现场变化、水印和业务背景，再给出确认重复、确认不同或待复核结论。")
    page_break(doc)
    # 15 innovation 2
    ...
    para(doc,"当处理对象从11万级反馈照片扩大到全电压等级月度千万级巡视照片，难点也随之变化：算法能够找出相似照片只是起点，整个月的数据能否稳定跑完才决定这项能力能不能长期使用。为此，计算路径按“特征预计算—低成本候选召回—高成本语义复核”分层，把真正需要精细比较的数据压缩到较小范围。")
    ...
    para(doc,"运行监测保留三类关键信号：任务是否持续推进、异常数据是否被记录、候选数量是否突然发生数量级变化。验证样本同时覆盖完全重复、水印修改、裁剪压缩、高相似但现场不同以及异常文件，既检查重复照片能否进入候选，也检查正常变化是否能留给人工终审。")
    ...
    para(doc,"这次排查形成的线路空间底座随后继续用于防鸟和集中燃放点。2025年年底集中燃放点任务到来后，仅需调整距离参数和筛选规则，算上修改调试不到半小时即可形成周边杆塔和涉及线路清单。同一批基础数据由一次性整理转为多专项复用，主业人员减少反复查资料，外协人员把更多时间留给现场核验。")
    ...
    para(doc,"其中最具代表性的是500千伏特殊通道事件：机器先从历史照片中筛出高相似线索，管理人员调取两次巡视原始记录逐项比对，确认同一区段重复使用照片，随后认定为人工巡视不到位，并定性为较大运检质量问题。这个事件说明，照片查重已经从“能找到相似照片”进入“能够支撑履职质量判断和管理闭环”的阶段。")
    h2(doc,"（三）管理方式：机器处理全量，人员负责判断")
    para(doc,"交叉跨越、防鸟和集中燃放点共用线路空间底座，告警工单和巡视照片共用照片智能筛查方法，两个方向最终都落到同一管理动作：机器处理规模大、重复度高的筛选工作，外协人员和主业人员集中完成现场确认、业务终审和问题处置。相关成果已经进入省公司实际管理流程，并多次形成生产管控中心管理通报。")
    para(doc,"推广条件也比较清晰：不依赖新增现场硬件，主要复用现有线路、杆塔、工单和照片数据。其他地市完成字段映射、坐标统一、规则参数配置和复核流程衔接后，即可复用现有方法。复制路径可以概括为三件事：接数据、配规则、走闭环。")
    paper_table(doc,"表10　案例实施成效与管理方式变化",("管理环节","传统方式","数智协同方式","实际成效"),(
        ("专项任务筛选","人员从全量对象中逐项寻找","空间分析先形成重点点位和区段清单","重要跨越排查由十余人、近两周压缩到数分钟，并补充发现十余处遗漏"),
        ("专项能力复用","不同专项重复整理线路与坐标","共用线路空间底座，仅调整业务规则","集中燃放点任务修改调试不到半小时即可形成核验清单"),
        ("外协履职监督","依赖有限抽查和人员历史记忆","照片全量筛查后集中复核异常候选","省公司试点形成19项照片重复问题，其中1项500千伏特殊通道问题定性为较大"),
    ...
    para(doc,"从应用路径看，这项工作经历了三次放大。第一次是把交叉跨越这种单项排查做成批量空间分析，解决一个具体任务；第二次是复用同一线路空间底座，将方法延伸到防鸟和集中燃放点，证明数据和规则可以跨专项复用；第三次是把照片筛查接入省公司实际管理，使数字化能力从“个人解决问题”进入“组织持续使用”的阶段。三个阶段对应了从单点提效、方法复用到管理机制沉淀的完整过程。")
    ...
    h2(doc,"结语")
    para(doc,"回看整个案例，有两组变化最能说明这条路径的价值。任务侧，一次原需十余人、近两周完成的交叉跨越排查，被压缩到数分钟完成第一轮全量计算，并补充发现十余处人工漏项；质量侧，照片查重从告警工单起步，随后扩展到巡视照片，并在省公司试点中发现500千伏特殊通道重复用图这一较大运检质量问题。两个场景一前一后，分别解决了“任务太多找不过来”和“照片太多查不过来”的现实矛盾。",14,False,together=True)
    para(doc,"由此形成的做法可以归纳为一句话：增效管任务、提质管履职。数字化工具负责处理规模大、重复度高的数据，外协和主业人员把精力放到现场确认、风险判断和问题处置。推广时按照“接数据、配规则、走闭环”的顺序完成本地化配置，即可在现有业务数据和管理流程上复用这套方法。",14,False,together=True)
