...
def build():
    ...
    para(doc,"从实践结果看，本案例的创新主要集中在三个层面。管理层面，把外协管理归纳为“增效管任务、提质管履职”两条主线；方法层面，把空间关系、距离条件和历史照片相似关系转成机器可执行的筛选规则；工程层面，把原有少量特高压照片筛查扩展到全电压等级，支撑每月1245万张巡视照片持续处理。三者共同服务同一目标：机器先处理全量数据，人员集中完成专业核验。")
    ...
    para(doc,"从组合规模看，111,519张照片若不附加任何业务约束，理论两两组合约62.18亿对，最终进入人工队列的只有5,472对。这个数量级只用于说明无约束搜索空间，不能理解为实际完成了62.18亿次高成本精细比对。候选生成的作用是尽早压缩搜索范围，把语义计算和人工复核集中到较小的数据集合。已复核候选中确认重复348对，占4,630对已复核候选的7.52%；这一指标反映候选队列的实际复核结果，不代表全部照片的重复率。")
    ...
    para(doc,"当处理对象从11万级反馈照片扩大到每月1245万张巡视照片，难点也随之变化：算法能够找出相似照片只是起点，整个月的数据能否稳定跑完才决定这项能力能不能长期使用。为此，计算路径按“特征预计算—低成本候选召回—高成本语义复核”分层，把真正需要精细比较的数据压缩到较小范围。")
    ...
    # 17 effects
    h1(doc,"四、实施效果")
    h2(doc,"（一）增效：外协拿到的是重点清单")
    para(doc,"变化最直观地体现在任务下达方式上。过去一项专项往往先给出大范围排查要求，外协人员还要自己找对象、查位置；现在先由数字化筛选工具把重点点位和区段筛出来，再按清单到现场确认。某地市重要跨越排查原需十余人、近两周完成，空间分析工具数分钟即可完成第一轮全量计算，同时补充发现十余处此前人工遗漏的重要跨越信息。")
    para(doc,"交叉跨越形成线路空间底座后，防鸟和集中燃放点继续复用同一批基础数据。主业人员减少了反复整理资料的时间，外协人员也把主要精力放到现场核验，任务组织从“大范围找目标”收敛为“围绕重点做确认”。")
    h2(doc,"（二）提质：海量照片先筛后看")
    ...
    para(doc,"过去照片问题主要靠抽查，发现重复很大程度受抽查范围和人员记忆限制。现在进入筛查范围的照片先统一比对，再把疑似照片对集中交给管理人员复核。19项照片重复问题进入省公司通报，其中还包含500千伏特殊通道较大运检质量问题，说明这套方法已经能够从海量历史照片中稳定提供可核验的问题线索。")
    h2(doc,"（三）管理方式：机器处理全量，人员负责判断")
    para(doc,"交叉跨越、防鸟和集中燃放点可以共用线路空间数据和筛选模块，告警工单与巡视照片查重也采用同一套候选筛选思路。相关核心算法和数字化工具均由我自主设计、开发和验证。告警工单反馈照片查重和巡视照片查重已经在省公司试点，发现的问题已多次进入省公司运检工作质量远程督查通报，说明这些数字化能力已经用于实际管理。")
