...
def build():
    ...
    para(doc,"上述问题的共同本质，是业务对象和历史数据规模已经超过人工全量处理能力，继续依赖增加人员和提高抽查频次既难以持续，也无法从根本上解决重复排查、漏项和质量核验不足等问题。因此，需要重新划分人机职责：机器承担全量筛选和重复劳动，人员集中负责现场核验、专业判断与管理闭环。")
    para(doc,"基于这一判断，我立足输电运检一线实际，自主开展数据梳理、规则设计和程序开发，将空间计算、图像相似度识别等技术与输电运检业务相结合，逐步形成“增效管任务、提质管履职”两条人机协同闭环。相关算法、计算程序和业务筛选流程均为自主设计、开发和验证成果，为后续形成可复制的外协数智管理模式奠定基础。")
    page_break(doc)
    # 6 模型
    h1(doc,"二、总体思路：构建“增效 + 提质”的外协数智管理模式")
    h2(doc,"（一）“增效管任务、提质管履职”双线协同")
    para(doc,"在前述实践基础上，我将自主开发的空间筛选和照片查重能力进一步归纳为“增效管任务、提质管履职”的双线管理模式：增效由系统从全量对象中筛选重点、外协精准核验；提质由系统对作业照片全量查重、管理人员重点复核异常候选。")
    ...
    h2(doc,"1.建立可复用的数据底座")
    para(doc,"首先对既有杆塔和线路数据进行清洗，检查空坐标、重复点、离群点和杆号顺序，将相邻杆塔连接为线路段；同时对铁路、鸟类活动、燃放点和气象节假日等外部数据统一标准化，将分散数据整理为可直接计算、可供不同专项重复调用的任务底座。")
    h2(doc,"2.把业务经验转化为可计算规则")
    para(doc,"在统一数据底座基础上，将“是否相交”“是否进入重点活动区域”“是否落入安全距离”等业务判断转化为空间叠加、风险分级和距离阈值，并编写对应计算逻辑，把个人经验固化为可重复执行的程序规则，批量生成重点核验清单。")
