...
def build():
    ...
    para(doc,"本案例把数据、规则、计算、复核和闭环连接起来。空间分析服务增效，照片全量比对服务提质，二者共同形成数智协同。")
    ...
    para(doc,"两个轮子共享一个轴心：标准化数据、明确规则、批量计算、人工复核和结果沉淀。增效缩短排查时间，提质扩大核验覆盖面。技术把人的精力集中到真正需要专业判断的环节。")
    page_break(doc)
    # 7 architecture
    h2(doc,"（二）数智协同的五层架构")
    ...
    para(doc,"三个增效场景采用统一流程：机器批量算出候选，人员再精准核验。排查范围明显收敛，现场确认标准保持不变。")
    page_break(doc)
    # 12 quality problem
    ...
    para(doc,"照片查重用于识别“现场构图和业务对象高度相似、但文件已经被处理”的疑似复用行为。程序同时分析低层视觉结构与高层语义，再由人员结合工单和巡视任务作最终认定。")
    ...
    para(doc,"本案例交付五个演示场景，并沉淀了业务人员自主定义规则、理解数据、设计验证、推动落地的完整路径。空间底座可以继续扩展到其他风险图层，照片特征与候选流程也可以复用到其他图像质量核查任务。")
