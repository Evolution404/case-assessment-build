...
def build():
    ...
    para(doc,"交叉跨越排查是这套数字化方法最早落地的业务场景。面对需要人员逐条线路、逐个区段查找的重要跨越信息，我利用既有全量杆塔坐标自主编写程序，将相邻杆塔连接形成线路空间模型，引入铁路、公路等地理要素，并采用“包围盒预筛＋精确几何求交”的批量计算方法，先快速排除明显无关的外部线要素，再对保留对象判断是否真正相交。")
    ...
    para(doc,"批量求交采用“包围盒预筛—精确求交”两级结构。实际程序先根据输电线路段的经纬度范围构造最小外包矩形，并向外缓冲0.0005°，利用空间相交关系从铁路、公路等外部线要素中筛出可能相关的对象；只有预筛结果进入后续精确几何求交。精确阶段使用Shapely的intersection计算真实交集，并分别处理点、多点、几何集合以及线状重合等结果。")
    paper_equation(doc,"intersection","1")
    para(doc,"式（1）用于说明有限线段求交的几何原理，其中A、B为输电线路段两个端点，C、D为外部线状要素对应线段的两个端点，t、u为各自线段参数；当两条参数线段存在共同点且参数落在0到1区间内时，交点位于两条有限线段上。实际程序的精确求交由Shapely几何运算完成，公式用于解释判定原理。",together=True)
    ...
    para(doc,"边界样本需要单独检查。精确求交结果可能是单个交点、多个交点或线状重合，程序按照几何类型分别处理；当交集为线状对象时，以交集中心位置作为后续核验参考。对于无几何对象、空几何和不符合目标类型的外部要素，则跳过并保留原始数据来源，避免异常对象干扰正式候选清单。")
    ...
    paper_table(doc,"表4　三类空间专项的计算规则与复用关系",("专项","空间关系","机器侧核心计算","人员侧核验重点"),(
        ("交叉跨越","线 × 线","包围盒预筛、Shapely精确几何求交","跨越类型、现场位置、管理属性"),
