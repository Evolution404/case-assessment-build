...
("1:20—2:00","数字化双轮飞轮","等待飞轮中心出现，再依次指向五个节点","我把这套方法设计成增效轮和提质轮共同驱动的飞轮。增效轮对应交叉跨越、防鸟、集中燃放点，用机器批量算，减少人海排查；提质轮对应告警工单和巡视照片查重，用全量比对验证照片真实性。每跑一轮，机器先把全量任务算完，人员把专业判断写回；下一轮数据更全、规则更准、处置更快。"),
...
("3:20—4:00","增效轮：空间计算替代人海排查","指出线、点、面三类对象","增效轮有三个场景，底层共用一套方法。交叉跨越计算线和线的交点；防鸟把鸟害点聚成重点区域，再和线路叠加；集中燃放点计算点到线路的距离。三类计算都落在同一套坐标底座上。下面我用全省脱敏地图现场演示。"),
...
("6:15—6:55","增效成果","说明程序输出候选，人员负责现场确认","空间计算把全量排查交给程序，现场人员负责确认图层误差、现场条件和真实风险。程序先把范围收敛，外协人员带着候选清单精准核验。效率提高了，现场确认标准保持不变。"),
...
("13:15—15:00","结论与推广","飞轮持续转动时放慢语速，最后一句停顿","回到这项案例的两条主线。增效，让交叉跨越、防鸟和燃放点排查从人海摸排转向机器计算、人员核验；提质，让告警工单和巡视照片从有限抽查转向全量验证。让双轮持续转起来，工作就会越做越快，结果越做越真。更重要的是，我完成了两个从无到有、从小到大的突破：独创告警工单查重，并把巡视照片查重扩展到全电压等级、每月千万级。机器承担重复劳动，专业判断回到关键环节。我的汇报完毕，谢谢各位评委。")]
